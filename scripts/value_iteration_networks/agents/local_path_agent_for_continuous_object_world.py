...
class LocalPlanAgent:
    ...
    def get_future_trajectory(self, position, orientation):
        for i_action in self.env.continuous_action_list:
            ...
            while t <= self.evaluation_time:
                ...
                if not out_of_range and not collision:
    ...
    def evaluation_local_path(self, local_position_list, local_orientation_list, global_state_list):
        ...
        obstacle_distance = [0 for i in xrange(self.env.n_continuous_action)]
        diffarence_heading = [0 for i in xrange(self.env.n_continuous_action)]
        velocity_list = [0 for i in xrange(self.env.n_continuous_action)]
        
        for i_action in self.env.continuous_action_list:
            if len(local_position_list[i_action]) > 0:
                ...
                #  #  print "tmp_dist_list : ", tmp_dist_list
                #  min_index = np.asarray(tmp_global_dist_list).argsort()
                #  #  print "min_index : ", min_index
                #  global_position_1 = global_state_list[np.where(min_index==0)][0]
                #  global_position_2 = global_state_list[np.where(min_index==2)][0]
                ...
                #  if L == 0.0:
                    #  L = 0.000001
                #  inverse_global_path_distance[i_action] = 1.0 / L
                ...
                obstacle_distance[i_action] = max_obs_dist
                ...
                diffarence_heading[i_action] = np.pi -  math.fabs(goal_orientation - final_orientation)
                #  print "diffarence_heading : ", math.degrees(diffarence_heading[i_action])
        ...
        print "diffarence_heading : ", diffarence_heading
        ...
        self.evaluation_value = b * np.asarray(obstacle_distance) \
                              + c * np.asarray(diffarence_heading) \
                              + d * np.asarray(velocity_list)
        #  self.evaluation_value = a * np.asarray(inverse_global_path_distance) \
                              #  + b * np.asarray(obstacle_distance) \
                              #  + c * np.asarray(diffarence_heading) \
                              #  + d * np.asarray(velocity_list)
