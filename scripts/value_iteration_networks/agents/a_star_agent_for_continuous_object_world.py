...
class AstarAgent:
    ...
    def create_heuristic(self):
        ...
        self.heuristic[discreate_goal] = 0
        for i in xrange(self.env.n_state):
            discreate_state = tuple(self.env.index2state(i))
            #  print "discreate_state : ", discreate_state
            diff_y = math.fabs(discreate_goal[0] - discreate_state[0])
            diff_x = math.fabs(discreate_goal[1] - discreate_state[1])
            #  print "diff_y, diff_x : ", diff_y, diff_x
            self.heuristic[discreate_state] = diff_y + diff_x
    ...
    def show_path(self):
        ...
        for i in xrange(len(state_list)):
            ...
            discreate_goal \
                    = self.env.continuous2discreate(self.env.goal[0], self.env.goal[1])
            if tuple(state_list[i]) == discreate_goal:
                vis_path[discreate_goal] = 'G'
