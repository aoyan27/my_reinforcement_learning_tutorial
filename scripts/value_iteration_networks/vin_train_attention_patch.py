...
from networks.vin_attention_patch import ValueIterationNetworkAttention
...
def view_map(array, title):
    fig = plt.figure()
    ax = fig.add_subplot(111)
    image = cv.cvtColor(array.astype(np.uint8), cv.COLOR_GRAY2RGB)
    im = ax.imshow(255 - 255*image, interpolation="nearest")
    fig.colorbar(im)
    plt.show()
...
def train_and_test(model, optimizer, gpu, model_path, train_data, test_data, n_epoch, batchsize):
    ...
    while epoch <= n_epoch:
        ...
        for i in xrange(0, n_train, batchsize):
            ...
            #  view_map(model.v.data[0, 0], 'state_value')
            loss.backward()
            optimizer.update()
            #  print "loss.grad : ", loss.grad
            #  print "model.l4.W.grad : ", model.l4.W.grad
            #  print "model.conv1.W.grad : ", model.conv1.W.grad
...
def main(dataset, n_epoch, batchsize, gpu, model_path, load_model_path):
    ...
    model = ValueIterationNetworkAttention(l_q=9, n_out=9, k=20, net=load_model_)
