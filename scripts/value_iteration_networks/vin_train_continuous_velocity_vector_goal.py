...
def load_dataset(path):
    ...
    return image_data, reward_map_data, goal_list_data, \
...
def train_test_split(image_data, reward_map_data, goal_list_data, \
                     position_list_data, orientation_list_data, \
                     action_list_data, velocity_vector_list_data, \
                     test_size, seed=0):
    np.random.seed(seed)
    ...
    goal_list_test = goal_list_data[index_test]
    ...
    goal_list_train = goal_list_data[index_train]
    ...
    train_data['goal'] = goal_list_train  
...
def train_and_test(model, optimizer, gpu, model_path, train_data, test_data, n_epoch, batchsize):
    ...
    while epoch <= n_epoch:
        ...
        for i in xrange(0, n_train, batchsize):
            ...
            batch_goal_list = train_data['goal'][perm[i:i+batchsize \
                    if i+batchsize < n_train else n_train]]
            ...
            if gpu >= 0:
                ...
                batch_goal_list = cuda.to_gpu(batch_goal_list)
            ...
            model.zerograds()
            loss, acc = model.forward(batch_input_data, batch_goal_list, \
        ...
        for i in xrange(0, n_test, batchsize):
            ...
            batch_goal_list = test_data['goal'][perm[i:i+batchsize \
                    if i+batchsize < n_test else n_test]]
...
def main(dataset, n_epoch, batchsize, gpu, model_path):
    image_data, reward_map_data, goal_list_data, position_list_data, orientation_list_data, \
    ...
    train_data, test_data \
            = train_test_split(image_data, reward_map_data, goal_list_data, \
                               position_list_data, orientation_list_data, \
