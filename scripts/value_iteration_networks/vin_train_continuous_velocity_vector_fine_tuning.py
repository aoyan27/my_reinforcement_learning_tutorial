...
from networks.vin_continuous_velocity_vector_fine_tuning import ContinuousValueIterationNetwork
...
def main(dataset, n_epoch, batchsize, gpu, model_path, load_model_path):
    ...
    model = ContinuousValueIterationNetwork(l_q=9, n_out=11, k=20, net=load_model_)
    #  model = ValueIterationNetwork(l_h=200, l_q=9, n_out=9, k=20)
    if gpu >= 0:
        cuda.get_device(gpu).use()
        model.to_gpu()
