...
def load_dataset(path):
    ...
    return image_data, reward_map_data, state_list_data, orientation_list_data, action_list_data

def train_test_split(image_data, reward_map_data, state_list_data, orientation_list_data, \
        action_list_data, test_size, seed=0):
    np.random.seed(seed)
...
def train_and_test(model, optimizer, gpu, model_path, train_data, test_data, n_epoch, batchsize):
    ...
    while epoch <= n_epoch:
        ...
        model_name = 'vin_model_with_orientation_%d.model' % epoch
        print model_name
...
def main(dataset, n_epoch, batchsize, gpu, model_path):
    image_data, reward_map_data, state_list_data, \
            orientation_list_data, action_list_data = load_dataset(dataset)
