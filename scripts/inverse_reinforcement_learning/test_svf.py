...
def main(rows, cols, gamma, act_noise, n_trajs, l_traj, learning_rate, n_itrs):
    ...
    vi_agent.get_policy(reward_gt, deterministic=False)
    policy = vi_agent.policy
    print "policy : "
    print policy
    #  print vi_agent.policy.reshape([rows, cols]).transpose()
    #  print vi_agent.policy.reshape(-1)
    env.show_policy(policy, deterministic=False)
    ...
    #  mu_D = deep_maxent_irl.expart_state_visitation_frequencies()
    #  print "mu_D : "
    #  print mu_D.reshape([rows, cols]).transpose()

    #  mu_exp = deep_maxent_irl.expected_state_visitation_frequencies(vi_agent.policy, deterministic=True)
    #  print "mu_exp(deterministic) : "
    #  print mu_exp.reshape([rows, cols]).transpose()
    #  mu_exp = deep_maxent_irl.expected_state_visitation_frequencies(policy, deterministic=False)
    #  print "mu_exp(stochastic) : "
    #  print mu_exp.reshape([rows, cols]).transpose()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--l_traj', default=100, type=int, help='length fo trajectory')
    parser.add_argument('-lr', '--learning_rate', default=0.01, type=float, help='learning rate')
    parser.add_argument('-ni', '--n_itrs', default=30, type=int, help='number of iterations')
