...
from networks.multi_agent_vin_with_velocity_and_orientation \
        import ValueIterationNetwork

from networks.multi_agent_vin_with_velocity_and_orientation_corner_case_fine_tuning \
        import ValueIterationNetworkFineTuning
...
def train_test_split(image_data, agent_image_data, another_position_data, position_data, \
    ...
    position_test = position_data[:, index_test]
    ...
    position_train = position_data[:, index_train]
...
def train_and_test(model, optimizer, gpu, model_path, train_data, test_data, n_epoch, batchsize):
    ...
    while epoch <= n_epoch:
        ...
        for i in xrange(0, n_train, batchsize):
            ...
            for agent_id in xrange(1):
                for other_agent_id in xrange(num_agent):
                    if agent_id != other_agent_id:
                        #  print "agent_id : ", agent_id
                        #  print "other_agent_id : ", other_agent_id
                        model.cleargrads()
                        loss, acc = model.forward(batch_input_data[agent_id], \
                                                  batch_state_list[agent_id], \
                                                  batch_velocity_list[agent_id], \
                                                  batch_orientation_list[agent_id], \
                                                  batch_state_list[other_agent_id], \
                                                  batch_velocity_list[other_agent_id], \
                                                  batch_orientation_list[other_agent_id], \
                                                  batch_action_list[agent_id])
        ...
        print 'train mean loss={}, accuracy={}'\
                .format(sum_train_loss/(2*n_train), sum_train_accuracy/(2*n_train))
        ...
        print 'test mean loss={}, accuracy={}'\
                .format(sum_test_loss/(2*n_test), sum_test_accuracy/(2*n_test))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--dataset', \
            default='datasets/multi_agent_object_world_velocity_orientation_map_dataset.pkl', \
