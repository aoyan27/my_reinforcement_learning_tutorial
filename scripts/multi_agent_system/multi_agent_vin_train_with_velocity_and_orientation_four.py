...
from networks.multi_agent_vin_with_velocity_and_orientation_four import ValueIterationNetwork
...
def train_and_test(model, optimizer, gpu, model_path, train_data, test_data, n_epoch, batchsize):
    ...
    while epoch <= n_epoch:
        ...
        for i in xrange(0, n_train, batchsize):
            ...
            for p in agent_perm:
                #  print "agent_id : ", p[0]
                #  print "other_agent_id_1 : ", p[1]
                #  print "other_agent_id_2 : ", p[2]
                model.cleargrads()
                loss, acc = model.forward(batch_input_data[p[0]], \
                                          batch_state_list[p[0]], \
                                          batch_velocity_list[p[0]], \
                                          batch_orientation_list[p[0]], \
                                          batch_state_list[p[1]], \
                                          batch_velocity_list[p[1]], \
                                          batch_orientation_list[p[1]], \
                                          batch_state_list[p[2]], \
                                          batch_velocity_list[p[2]], \
                                          batch_orientation_list[p[2]], \
                                          batch_state_list[p[3]], \
                                          batch_velocity_list[p[3]], \
                                          batch_orientation_list[p[3]], \
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--dataset', \
            default='datasets/multi_agent_object_world_velocity_orientation_future_4_agents_map_dataset.pkl', \
