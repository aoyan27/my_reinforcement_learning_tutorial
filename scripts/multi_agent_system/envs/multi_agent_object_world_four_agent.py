...
class Objectworld:
    def __init__(self, rows, cols, n_objects, num_agent, noise,\
                 start={0: [0, 0], 1: [4, 4], 2: [4, 0], 3: [0, 4]}, \
                 goal={0: [4, 4], 1: [0, 0], 2: [0, 4], 3:[4, 0]}, \
        ...
        self.agent_grid = {}
        self.set_init_agent_grid()
        self.agent_grid_future = {}
        self.set_init_agent_grid_future()
        # +------------> x
        # |
        # |
        # |
        ...
        self.set_velocity(self.init_velocity)
    ...
    def set_init_agent_grid_future(self):
        for agent_id in xrange(self.num_agent):
            self.agent_grid_future[agent_id] = copy.deepcopy(self.agent_grid[agent_id])

    def set_init_agent_grid(self):
        for agent_id in xrange(self.num_agent):
            self.agent_grid[agent_id] = copy.deepcopy(self.grid)

    def set_velocity(self, input_velocity):
        for agent_id in xrange(self.num_agent):
            self.velocity[agent_id] = input_velocity
    ...
    def set_agent_grid_future(self):
        ...
        for i in xrange(self.num_agent):
            for j in xrange(self.num_agent):
                if i != j:
                    for k in xrange(len(self._future_state[j])):
                        #  print "i : ", i
                        #  print "j : ", j
                        #  print self._future_state[j][k]
                        self.agent_grid_future[i][tuple(self._future_state[j][k])] = -1
            #  print self.agent_grid_future[i].astype(np.int8)
    ...
    def set_future_state(self, future_state, init=False):
        if init:
            tmp_future_state = copy.deepcopy(future_state)
            for agent_id in xrange(self.num_agent):
                self._future_state[agent_id] = [tmp_future_state[agent_id]]
            #  print "self._future_state : ", self._future_state
        else:
            self._future_state = copy.deepcopy(future_state)
    ...
    def set_start(self, start):
        ...
        self.set_future_state(start, init=True)
        self.set_agent_grid()
    ...
    def get_future_state_multi_step(self, state, action, max_step=2):
        future_state_list = []
        future_state = state
        for i_step in xrange(max_step):
            future_state, out_of_range, collisions = self.move(future_state, action)
            future_state_list.append(future_state)
        #  print "future_state_list : ", future_state_list
        return future_state_list

    def reset(self, start_position={0: [0, 0], 1: [4, 4], 2: [4, 0], 3:[0, 4]}, \
                    goal_position={0: [4, 4], 1: [0, 0], 2: [0, 4], 3: [4, 0]}, random=False):
    ...
    def step(self, action):
        ...
        for i in xrange(self.num_agent):
            ...
            tmp_future_state_[i] \
                    = self.get_future_state_multi_step(self._state[i], action_index, max_step=1)
        #  print "tmp_future_state : ", tmp_future_state

        self.set_future_state(tmp_future_state_)
    ...
    def show_objectworld_with_state(self):
        ...
        for i in xrange(self.num_agent):
            grid[tuple(self.goal[i])] = i+6+1
            if self._state[i] != None:
                grid[tuple(self._state[i])] = i+1
        for row in grid:
            print "|",
            for i in row:
                if i == 0:
...
if __name__=="__main__":
    ...
    #  start = {0: [3, 3], 1: [11, 11], 2:[11, 2], 3: [2, 11]}
    #  goal = {0: [13, 13], 1:[0, 0], 2:[1, 13], 3: [13, 1]}

    start = {0: [2, 2], 1: [18, 18], 2:[17, 2], 3: [1, 18]}
    goal = {0: [19, 19], 1:[0, 0], 2:[0, 19], 3: [19, 0]}
    ...
    for i in xrange(100):
        ...
        for j in xrange(100):
            ...
            #  for agent_id in xrange(ow.num_agent):
                ...
                #  print ow.agent_grid[agent_id].astype(np.int8)
                #  print "#####################"
                #  print ow.agent_grid_future[agent_id].astype(np.int8)
                #  print "++++++++++++++++++++++++++++++"
            for agent_id in xrange(ow.num_agent):
