...
def load_dataset(path):
    with open(path, mode='rb') as f:
        data = None
        data = pickle.load(f)
    tmp_image_data = data['grid_image']
    tmp_reward_map_data = data['reward']
    tmp_state_list_data = data['state']
    tmp_action_list_data = data['action']

    image_data = tmp_image_data[0]
    reward_map_data = tmp_image_data[0]
    state_list_data = tmp_state_list_data[0]
    action_list_data = tmp_action_list_data[0]
    for i in xrange(1, len(tmp_image_data)):
        image_data = np.concatenate([image_data, tmp_image_data[i]], axis=0)
        reward_map_data = np.concatenate([reward_map_data, tmp_reward_map_data[i]], axis=0)
        state_list_data = np.concatenate([state_list_data, tmp_state_list_data[i]], axis=0)
        action_list_data = np.concatenate([action_list_data, tmp_action_list_data[i]], axis=0)
...
def train_test_split(image_data, reward_map_data, state_list_data, action_list_data, \
        test_size, seed=0):
    np.random.seed(seed)
    n_dataset = np.asarray(image_data).shape[0]
